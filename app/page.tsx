...
export default function HomePage() {
  ...
  const backupEnabled = process.env.NEXT_PUBLIC_DB_BACKUP_ENABLED === "true";

  async function createBackup() {
    setBackupState("loading");
    try {
      const res = await fetch("/api/backup", { method: "POST" });
      setBackupState(res.ok ? "done" : "error");
    } catch {
          ...
          <div className="flex items-center gap-4">
            {backupEnabled && (
              <button
                ...
                title="Create a backup of the database"
              >
                {backupState === "loading" ? (
                ...
                {backupState === "done" ? "Saved!" : backupState === "error" ? "Failed" : "Backup DB"}
              </button>
            )}
            <Link
              href="/quiz"
